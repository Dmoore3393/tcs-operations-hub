...
type EncryptedDocument = {
  ...
  status: string;
  children: { first_name?: string; last_name?: string; legacy_id?: string } | null;
  locations: { name?: string; full_name?: string; slug?: string } | null;
};
...
function isLeadership(fullName = "", email = "") {
...
export default function TransportationConsentsPage() {
  ...
  const canManage = isSystemOwner || isLocationLicensee || isLeadership(profile?.full_name, profile?.email);

  const eligibleChildren = useMemo(() => (Array.isArray(children) ? children : []).filter((child) => {
    const schedule = (Array.isArray(childSchedules) ? childSchedules : []).find((item) => item.childId === child.id);
  ...
  const loadDocuments = useCallback(async () => {
    ...
    setMessage("");
    try {
      const response = await fetch(`/api/transportation-consents/documents?location=${encodeURIComponent(location)}`, { headers: { Authorization: `Bearer ${session.access_token}` }, cache: "no-store" });
  ...
  function recordFor(child: ChildRecord) {
    const name = childName(child).trim().toLowerCase();
    return consentDocuments.find((document) => `${document.children?.first_name ?? ""} ${document.children?.last_name ?? ""}`.trim().toLowerCase() === name);
  }

  function hasConsent(child: ChildRecord) {
    return Boolean(recordFor(child));
  }
  ...
  async function copyLink() {
    ...
    try {
      ...
      setMessage("Parent signing link copied. You can paste it into a text or Brightwheel message.");
    } catch {
      setMessage("The link is shown below. Press and hold it to copy.");
    }
  }
  ...
  async function uploadExisting(event: FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/transportation-consents/documents", { method: "POST", headers: { Authorization: `Bearer ${session.access_token}` }, body: form });
  ...
  async function download(document: EncryptedDocument) {
    ...
    setMessage("");
    try {
      const response = await fetch(`/api/transportation-consents/documents/${document.id}`, { headers: { Authorization: `Bearer ${session.access_token}` } });
      if (!response.ok) {
        const payload = await response.json() as { error?: string };
        throw new Error(payload.error || "Could not open the signed consent.");
      }
      ...
      setMessage(error instanceof Error ? error.message : "Could not open the signed consent.");
    }
  }

  if (!canManage) return <MainLayout><div className="mx-auto max-w-3xl rounded-3xl border border-amber-200 bg-amber-50 p-8"><h1 className="text-2xl font-black text-amber-950">Transportation Consent Forms</h1><p className="mt-3 font-semibold text-amber-900">Signed consent records are limited to authorized transportation leadership and Location Licensees because they contain parent signatures and private family information.</p><Link href="/transportation" className="mt-5 inline-flex rounded-xl bg-[#102a56] px-4 py-3 text-sm font-black text-white">Back to Transportation</Link></div></MainLayout>;
  ...
  return <MainLayout><div className="mx-auto max-w-[1500px] space-y-6 pb-12">
    <section className="overflow-hidden rounded-[2rem] border border-blue-100 bg-gradient-to-br from-[#eef7ff] via-white to-[#fff4bf] p-6 shadow-[0_18px_55px_rgba(19,76,145,.14)] sm:p-9">
      ...
      <div className="mt-2 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><h1 className="text-4xl font-black tracking-tight text-[#102a56] sm:text-5xl">Transportation Consent Forms</h1><p className="mt-3 max-w-3xl text-sm font-semibold leading-7 text-slate-600">Create a secure parent signing link or upload a consent form that is already signed. Completed records stay attached to the child in encrypted storage.</p></div><Link href="/transportation" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-blue-200 bg-white px-4 py-3 text-sm font-black text-[#1769d2]">Back to Transportation</Link></div>
    </section>
    ...
    <SectionCard title="Choose child" description="Select the correct location and child before creating a signing link or uploading an existing signed form.">
    ...
    <section className="grid gap-5 xl:grid-cols-2">
      <SectionCard title="Digital Parent Signature" description="The parent completes the 2026–2027 packet on their phone, initials the required sections, and draws their signature.">
        <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm font-semibold leading-6 text-blue-950"><ShieldCheck className="mr-2 inline h-4 w-4" />The link is child-specific and expires after 7 days.</div>
      ...
      <SectionCard title="Upload Existing Signed Form" description="Use this for the signed paper/PDF consent forms you already have.">
        <form onSubmit={uploadExisting} className="space-y-4"><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold leading-6 text-emerald-950"><CheckCircle2 className="mr-2 inline h-4 w-4" />The PDF or image is encrypted and stored only as a Transportation Consent 2026–2027 record.</div><label className="block"><span className="mb-1.5 block text-sm font-bold text-slate-700">Signed PDF or image</span><input id="existing-transport-consent" required type="file" accept=".pdf,.jpg,.jpeg,.png,.heic,.heif,application/pdf,image/jpeg,image/png,image/heic,image/heif" className={inputClass} onChange={(event) => setUploadFile(event.target.files?.[0] ?? null)} /></label><PrimaryButton type="submit" disabled={!selectedChild || !uploadFile || uploading}>{uploading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />} Encrypt & Upload Signed Form</PrimaryButton></form>
      </SectionCard>
    </section>
