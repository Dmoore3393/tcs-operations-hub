...
import { locationKeyForDbLocation, type StaffShiftRow, type StaffingLocation } from "@/lib/staffing-command";
...
export default function MySchedulePage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    const [locationResult, shiftResult, publicationResult] = await Promise.all([
      ...
      supabase
        ...
        .select("id,location_id,week_of,status,revision,published_at,published_by,notes")
        .eq("week_of", weekStart)
        .eq("status", "Published"),
    ]);

    const failure = locationResult.error || shiftResult.error || publicationResult.error;
    if (failure) {
  ...
  const scheduledLocationIds = useMemo(() => [...new Set(shifts.map((item) => item.location_id))], [shifts]);
  ...
  return <MainLayout><div className="mx-auto max-w-[1450px] space-y-6 pb-12">
    ...
    <section className="grid gap-4 sm:grid-cols-3">
      <StatCard label="Published Shifts" value={shifts.length} helper="This week" icon={<CalendarDays className="h-5 w-5" />} tone="blue" />
