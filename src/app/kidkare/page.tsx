...
export default function KidKarePage() {
  ...
  function removeEnrollment(record: KidKareEnrollment) {
    const confirmed = window.confirm(`Remove ${record.childName} from the KidKare tracker for ${record.location}?\n\nThis removes only this KidKare enrollment record. It will NOT delete the child or family profile from TCS Operations Hub.`);
    if (!confirmed) return;
    setRecords((current) => current.filter((item) => item.id !== record.id));
    if (editing?.id === record.id) closeModal();
    setShowSuccess(true);
    window.setTimeout(() => setShowSuccess(false), 2200);
  }
