...
export default function ParentAccountSetupPage() {
  ...
  useEffect(() => {
    ...
    setMode(search.get("mode") === "recovery" ? "recovery" : "invite");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (mode === "recovery") {
        const statusResponse = await fetch("/api/parent/account-status", {
        ...
        if (!statusResponse.ok) throw new Error(status.error || "Could not verify your Parent Portal account.");
        setComplete(true);
        window.setTimeout(
          () => router.replace(status.hasActiveAccess ? "/parent" : status.pendingApprovalCount ? "/parent/account-pending" : "/parent-login"),
          900,
        );
        return;
      }
      ...
      if (!response.ok) throw new Error(payload.error || "Your Parent Portal account could not be submitted for approval.");

      setComplete(true);
      window.setTimeout(() => router.replace("/parent/account-pending"), 900);
    } catch (caught) {
    ...
    <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-5xl items-center justify-center">
      <section className="grid w-full overflow-hidden rounded-[34px] border border-slate-200 bg-white shadow-2xl lg:grid-cols-[.9fr_1.1fr]">
        <div className="bg-gradient-to-br from-[#173d29] via-[#245a39] to-[#10291e] p-7 text-white sm:p-10">
          ...
          <h1 className="mt-2 text-4xl font-black">{mode === "recovery" ? "Reset your password" : "Create your account"}</h1>
          <p className="mt-4 text-sm font-semibold leading-7 text-emerald-50/80">{mode === "recovery" ? "Choose a new password for your existing Parent Portal account." : "Your TCS location invited this email to the Parent Portal. You choose your own password; TCS staff do not create or know it."}</p>
        ...
        <div className="p-7 sm:p-10">
          {complete ? <div className="py-12 text-center">
            ...
            <h2 className="mt-4 text-3xl font-black text-slate-950">{mode === "recovery" ? "Password updated" : "Account setup complete"}</h2>
            <p className="mt-3 text-sm font-semibold text-slate-600">{mode === "recovery" ? "Taking you back to your account…" : "Your access is now waiting for TCS staff approval."}</p>
          </div> : <>
            ...
            <form onSubmit={submit} className="mt-7 space-y-4">
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-semibold leading-5 text-emerald-950">{mode === "recovery" ? "Your new password will replace the old one for this Parent Portal account." : "By finishing setup, you are creating your own authenticated account. Child information stays locked until TCS staff reviews and approves your access settings."}</div>

              <button disabled={saving} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#245a39] px-4 py-3 text-sm font-black text-white disabled:opacity-50">{saving ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />} {mode === "recovery" ? "Update My Password" : "Create My Parent Account"}</button>
            </form>
