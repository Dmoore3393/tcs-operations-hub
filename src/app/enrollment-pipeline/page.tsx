...
export default function TourBoardPage() {
  ...
  return <div className="tour-shell">
    ...
    {leadDraft&&<ModalShell title={leads.some(item=>item.id===leadDraft.id)?"Update Family Lead":"Add New Childcare Lead"} subtitle="CRM-style family record: who they are, what they need, what happened last, and what happens next." onClose={()=>setLeadDraft(null)} wide>
      <div className="tour-crm-form-title">Family & Contact</div>
      <div className="tour-form">
        <Field label="Location"><select value={leadDraft.location} onChange={(e)=>setLeadDraft({...leadDraft,location:e.target.value})}>{availableLocations.filter(item=>item!=="All Locations").map(item=><option key={item}>{item}</option>)}</select></Field>
        <Field label="Pipeline status"><select value={leadDraft.stage} onChange={(e)=>setLeadDraft({...leadDraft,stage:e.target.value as TourBoardStage})}>{[...TOUR_BOARD_COLUMNS,"Declined" as const].map(item=><option key={item}>{item}</option>)}</select></Field>
        <Field label="Family name"><input value={leadDraft.familyName} onChange={(e)=>setLeadDraft({...leadDraft,familyName:e.target.value})} placeholder="Family last name"/></Field>
        <Field label="Parent / guardian"><input value={leadDraft.parentName} onChange={(e)=>setLeadDraft({...leadDraft,parentName:e.target.value})}/></Field>
        <Field label="Phone"><input value={leadDraft.phone} onChange={(e)=>setLeadDraft({...leadDraft,phone:e.target.value})}/></Field>
        <Field label="Email"><input type="email" value={leadDraft.email} onChange={(e)=>setLeadDraft({...leadDraft,email:e.target.value})}/></Field>
        <Field label="Best contact time"><input value={leadDraft.bestContactTime} onChange={(e)=>setLeadDraft({...leadDraft,bestContactTime:e.target.value})} placeholder="Example: After 4 PM"/></Field>
        <Field label="How they contacted us"><select value={leadDraft.inquiryMethod} onChange={(e)=>setLeadDraft({...leadDraft,inquiryMethod:e.target.value as ContactMethod})}>{contactMethods.map(item=><option key={item}>{item}</option>)}</select></Field>
      </div>

      <div className="tour-crm-form-title">Childcare Need</div>
      <div className="tour-form">
        <Field label="Child name"><input value={leadDraft.childName} onChange={(e)=>setLeadDraft({...leadDraft,childName:e.target.value})}/></Field>
        <Field label="Child age"><input value={leadDraft.childAge} onChange={(e)=>setLeadDraft({...leadDraft,childAge:e.target.value})} placeholder="3y 4m"/></Field>
        <Field label="Additional children / siblings"><input value={leadDraft.additionalChildren} onChange={(e)=>setLeadDraft({...leadDraft,additionalChildren:e.target.value})} placeholder="Names / ages if applicable"/></Field>
        ...
        <label className="tour-check"><input type="checkbox" checked={leadDraft.transportationNeeded} onChange={(e)=>setLeadDraft({...leadDraft,transportationNeeded:e.target.checked})}/>Transportation needed</label>
      </div>

      <div className="tour-crm-form-title">CRM Follow-Up</div>
      <div className="tour-form">
        <Field label="Lead source"><select value={leadDraft.leadSource} onChange={(e)=>setLeadDraft({...leadDraft,leadSource:e.target.value as LeadSource})}>{sources.map(item=><option key={item}>{item}</option>)}</select></Field>
        <Field label="Assigned to"><input value={leadDraft.assignedTo} onChange={(e)=>setLeadDraft({...leadDraft,assignedTo:e.target.value})}/></Field>
        <Field label="Priority / temperature"><select value={leadDraft.priority} onChange={(e)=>setLeadDraft({...leadDraft,priority:e.target.value as TourBoardLead["priority"]})}><option>Normal</option><option>Hot Lead</option><option>Urgent</option></select></Field>
        <Field label="Follow-up date"><input type="date" value={leadDraft.followUpDate} onChange={(e)=>setLeadDraft({...leadDraft,followUpDate:e.target.value})}/></Field>
        <Field label="Next action" full><input value={leadDraft.nextAction} onChange={(e)=>setLeadDraft({...leadDraft,nextAction:e.target.value})} placeholder="Call tomorrow, send rates, schedule tour, follow up on packet..."/></Field>
        <Field label="Tags" full><input value={(leadDraft.tags||[]).join(", ")} onChange={(e)=>setLeadDraft({...leadDraft,tags:e.target.value.split(",").map(tag=>tag.trim()).filter(Boolean)})} placeholder="Sibling Enrollment, Extended Hours, Waiting on Documents..."/></Field>
        <Field label="General notes" full><textarea value={leadDraft.notes} onChange={(e)=>setLeadDraft({...leadDraft,notes:e.target.value})}/></Field>
        {leadDraft.stage==="Waitlist"&&<Field label="Waitlist reason" full><textarea value={leadDraft.waitlistReason} onChange={(e)=>setLeadDraft({...leadDraft,waitlistReason:e.target.value})}/></Field>}
        {leadDraft.stage==="Declined"&&<Field label="Why they did not enroll" full><textarea value={leadDraft.declinedReason} onChange={(e)=>setLeadDraft({...leadDraft,declinedReason:e.target.value})} placeholder="Price, hours, no space, chose another provider, unable to reach, subsidy issue..."/></Field>}
      </div>
      <div className="tour-modal-actions"><button className="tour-cancel" onClick={()=>setLeadDraft(null)}>Cancel</button><button className="tour-save" onClick={saveLead}>Save Lead</button></div>
    </ModalShell>}
