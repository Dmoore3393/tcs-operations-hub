  ...
  return <MainLayout><div className="woodland-center mx-auto w-full max-w-[1500px] text-[#293423]"><div className="woodland-inner space-y-3 pb-3">
    ...
    <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_330px]">
      <div className="space-y-3">
        ...
        <section className="flex flex-wrap gap-3">{canManage && <button onClick={openAdd} className="inline-flex items-center gap-2 rounded-xl bg-[#445b37] px-5 py-3 text-sm font-black text-white"><Plus className="h-4 w-4" /> Add Child</button>}{canManage && <Link href="/child-file-audits" className="inline-flex items-center gap-2 rounded-xl bg-[#9f2f20] px-5 py-3 text-sm font-black text-white"><FileWarning className="h-4 w-4" /> Child File Audits</Link>}{canManage && <Link href="/immunization-tracker" className="inline-flex items-center gap-2 rounded-xl bg-[#2e6c47] px-5 py-3 text-sm font-black text-white"><HeartPulse className="h-4 w-4" /> Shot Records</Link>}{canManage && <Link href="/school-age-support" className="inline-flex items-center gap-2 rounded-xl bg-[#174b7c] px-5 py-3 text-sm font-black text-white"><CalendarClock className="h-4 w-4" /> School Age Support</Link>}<Link href="/emergency-cards" className="inline-flex items-center gap-2 rounded-xl bg-[#a83232] px-5 py-3 text-sm font-black text-white"><HeartPulse className="h-4 w-4" /> Emergency Medical Cards</Link><Link href="/enrollment-pipeline" className="inline-flex items-center gap-2 rounded-xl bg-[#b95d3b] px-5 py-3 text-sm font-black text-white"><FileWarning className="h-4 w-4" /> Start Enrollment</Link><Link href="/child-schedules" className="inline-flex items-center gap-2 rounded-xl border border-[#e6dfd4] bg-[#fffaf2] px-5 py-3 text-sm font-black"><CalendarClock className="h-4 w-4" /> Child Schedules</Link><button onClick={exportRoster} className="inline-flex items-center gap-2 rounded-xl border border-[#e6dfd4] bg-[#fffaf2] px-5 py-3 text-sm font-black"><Download className="h-4 w-4" /> Export Roster</button></section>
      </div>
    ...
    {selected && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4">
      ...
      <section className="relative z-10 w-full max-w-2xl rounded-3xl bg-[#fffdf9] p-6 shadow-2xl">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <Link href={`/emergency-cards?child=${selected.id}`} className="inline-flex items-center gap-2 rounded-xl bg-[#a83232] px-4 py-2.5 text-sm font-black text-white"><HeartPulse className="h-4 w-4" /> Emergency Card</Link>{canManage && keyFor(selected.location) !== "Tehachapi" && <Link href={`/child-file-audits?child=${selected.id}`} className="inline-flex items-center gap-2 rounded-xl bg-[#9f2f20] px-4 py-2.5 text-sm font-black text-white"><FileWarning className="h-4 w-4" /> File Audit</Link>}{canManage && <Link href={`/immunization-tracker?child=${selected.id}`} className="inline-flex items-center gap-2 rounded-xl bg-[#2e6c47] px-4 py-2.5 text-sm font-black text-white"><HeartPulse className="h-4 w-4" /> Shot Record</Link>}{canManage && selected.ageGroup === "School Age" && <Link href="/school-age-support" className="inline-flex items-center gap-2 rounded-xl bg-[#174b7c] px-4 py-2.5 text-sm font-black text-white"><CalendarClock className="h-4 w-4" /> School Support</Link>}
