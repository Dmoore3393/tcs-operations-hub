    ...
    {selected && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4">
      ...
      <section className="relative z-10 w-full max-w-2xl rounded-3xl bg-[#fffdf9] p-6 shadow-2xl">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <Link href={`/emergency-cards?child=${selected.id}`} className="inline-flex items-center gap-2 rounded-xl bg-[#a83232] px-4 py-2.5 text-sm font-black text-white"><HeartPulse className="h-4 w-4" /> Emergency Card</Link>{canManage && keyFor(selected.location) !== "Tehachapi" && <Link href={`/child-file-audits?child=${selected.id}`} className="inline-flex items-center gap-2 rounded-xl bg-[#9f2f20] px-4 py-2.5 text-sm font-black text-white"><FileWarning className="h-4 w-4" /> File Audit</Link>}{canManage && <Link href={`/immunization-tracker?child=${selected.id}`} className="inline-flex items-center gap-2 rounded-xl bg-[#2e6c47] px-4 py-2.5 text-sm font-black text-white"><HeartPulse className="h-4 w-4" /> Shot Record</Link>}
